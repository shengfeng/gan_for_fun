...
parser.add_argument('--datasets', metavar='N', type=str, nargs='+', choises=['MNIST', 'SVHN', 'CIFAR10'])
...
def dowmload_svhn(download_path):
...
def download_cifar10(download_path):
    ...
    train_image = np.reshape(np.stack(train_image, axis=0), [num_cifar10_train, 32*32*3])
    train_label = np.reshape(np.array(np.stack(train_label, axis=0)), [num_cifar10_train])


    fd = os.path.join(target_path, 'test_batch')
    dict = unpickle(fd)
    test_image = np.reshape(dict['data'], [num_cifar10_test, 32 * 32 *3])
    test_label = np.reshape(dict['labels'], [num_cifar10_test])
